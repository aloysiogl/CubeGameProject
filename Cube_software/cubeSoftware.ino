...
//Cube inital setup
void setup(){
  pinMode(latch,OUTPUT);  //Setting shift register pins
  pinMode(data,OUTPUT);
  pinMode(clk,OUTPUT);
  Serial.begin(115200);
}
...
void loop(){
  ...
  //Connectiong animations
  Connecting1_(850);
  Connecting2_(850);

  //If connection is ok read serial data and execute commands
  if (isConnected())
   connection = 1;
  while (connection){
      readSerial();
...
//Veryfies connection (handshaking)
int isConnected(){
    bool validation = 0;
    char reading;
...
static void exe_led(int duration){
  ...
  //Keeps an animation frame for the desired duraton
  unsigned long counter = millis() + duration;

  while(counter >= millis()){
    //Torns on each layer at a time
...
//Writes the commands directly to hte cube
static void exe(){
  //Writes output and closes the latch pin
...
/****************************************************
This part of the code has several in-built animations
****************************************************/

//Auxiliary code for and animation
...
//Special tipy of rain animation
void a(unsigned int duration){

//keeps a frame for the desired time
unsigned long counter = millis() + duration;
 while(counter >= millis()){
   for(int l=0; l < 5; l++){
...
//One kind of snake animation
void snake1 (unsigned int duration){
  for (int k=0; k <125;k++)
    led[k]=0;

  //Turns on the initial LED
  const int siZe = 3;
...
//Another kind of snake animation
...
//Keeps the frame for the desired time
...
//Stars blinking effect on the cube
...
//Intercalates three different sized cubes
