...
Serial::Serial(int portNumber, bool trying)
{
    //We're not yet connected
    this->connected = false;
    ...
    //Try to connect to the given port throuh CreateFile
    this->hSerial = CreateFile(namePort,
            GENERIC_READ | GENERIC_WRITE,
            0,
            NULL,
            OPEN_EXISTING,
    ...
    if(this->hSerial==INVALID_HANDLE_VALUE)
    {
        ...
        if(GetLastError()==ERROR_FILE_NOT_FOUND){
            ...
            //Print Error if neccessary
            if (trying)
                 printf("COM%d not available trying next...\n", portNumber);
            else printf("ERROR: Handle was not attached. Reason: COM%d not available.\n", portNumber);

        }
        else
        {
            printf("Error at COM%d!!! \n", portNumber);
        }
    }
    else
    {
...
http://www.halcon.com/download/documentation/reference-9.0/cpp/read_serial.html
http://www.codeguru.com/cpp/i-n/network/serialcommunications/article.php/c5425/Serial-Communication-in-Windows.htm
https://msdn.microsoft.com/pt-br/library/aa450604.aspx
...
https://batchloaf.wordpress.com/2013/02/12/simple-trick-for-sending-characters-to-a-serial-port-in-windows/     (echo)
